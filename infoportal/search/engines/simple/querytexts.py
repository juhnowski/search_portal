...
from .buildindex import BuildIndex, stopwords
...
class Query:
    ...
    def __init__(self):
        self.index = BuildIndex().load(EXT_NAME)
    ...
    def one_word_query(self, word):
        ...
        if word in self.invertedIndex.keys():
            return self.rankResults(
                [filename for filename in self.invertedIndex[word].keys()],
                word)
        else:
            return []
    ...
    def query_vec(self, query):
        ...
        final = [tf[i] * queryidf[i] for i in range(
            len(self.index.getUniques()))]
        return final
...
if __name__ == '__main__':
    q = Query(r"../db.sqlite3", EXT_NAME)
    print(q.one_word_query("вид"))
    print(q.phrase_query("вид"))
