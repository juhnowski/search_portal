...
if settings.SEARCH_ENGINE.get('name') == 'solr':
    CLIENT = SolrClient(settings.SEARCH_ENGINE.get('URL'))
    res = CLIENT.query('infoportal', {
        'q': 'doc_name_ru:акустика',
        'facet': True,
        'facet.field': 'doc_name_ru',
    })
    print(f"count = {res.get_results_count()} docs={res.docs}")
...
def phrase_query(s_text):
    ...
    for d in res.docs:
        doc.append(d.get('doc_id')[0])
    return doc


def solr_search_auto_completions(search_serializer):
...
def solr_search_text(search_serializer, page, page_size, user_id):
    s_text = search_serializer.data['search_text']
