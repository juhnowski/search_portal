...
if settings.SEARCH_ENGINE.get('name') == 'sphinx':
    ...
    CLIENT.SetServer(
        settings.SEARCH_ENGINE.get('HOST'),
...
#
# @timing
def phrase_query(s_text):
    p = CLIENT.Query(s_text)
    doc = []
    COUNT_SPHINX_SEARCH = p.get('docs')
    for d in p.get('matches'):
        doc.append(d.get('id'))
    return doc
...
#     client = sphinxapi.SphinxClient()
#     client.SetServer('127.0.0.1', 33959)
#     doc = []
#     for d in p.get('matches'):
#         doc.append(d.get('id'))
#     print(doc)
...
def sphinx_search_auto_completions(search_serializer):
...
def sphinx_search_text(search_serializer, page, page_size, user_id):
    s_text = search_serializer.data['search_text']
    ...
    if s_text:
        if CLIENT:
            # serched = phrase_query(s_text.lower())

            p = CLIENT.Query(s_text.lower())
            serched = []
            COUNT_SPHINX_SEARCH = p.get('total')
            print(f"COUNT_SPHINX_SEARCH={p.get('total')}")
            # print(f'p={p}')
            for d in p.get('matches'):
                serched.append(d.get('id'))
            ...
            for docId in doc_set:
                ...
                if len(result) == top_limit:
                    return JsonResponse({'documents': result[(page - 1) * page_size:], 'count': COUNT_SPHINX_SEARCH})

            # count = len(result)
            # limit_for_free_search = top_limit - len(result)
            # doc_set = set()
            # if limit_for_free_search > 0:
            #     free_serched = phrase_query(s_text.lower())
            ...
            if len(result) < (page - 1) * page_size:
                return JsonResponse({'documents': [], 'count': COUNT_SPHINX_SEARCH})
            else:
                return JsonResponse({'documents': result[(page - 1) * page_size:], 'count': COUNT_SPHINX_SEARCH})
    else:
        if s_options:
