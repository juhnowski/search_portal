...
@timing
def Query(text):
    QUERY_DOC_NAME_RU.parse(text)
    return CLIENT.search(QUERY_DOC_NAME_RU)


def phrase_query(s_text):
    doc = []
    for d in Query(s_text):
        doc.append(d.get('doc_id'))
    return doc


def whoosh_search_auto_completions(search_serializer):
...
def whoosh_search_text(search_serializer, page, page_size, user_id):
    s_text = search_serializer.data['search_text']
